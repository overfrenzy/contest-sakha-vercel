export default async function handler(req, res) {
    ...
    try {
      const response = await fetch('https://functions.yandexcloud.net/d4e96bpn267cvipclv1f');
      const data = await response.json();
  
      const contestPage = data.contests.find((contest) => contest.name === contestName);
      ...
      const participations = filteredParticipations.map((participation) => {
        ...
        const participant = data.participants.find(
          (participant) => participant.participant_id === participation.participant_id
        );
        const country = data.countries.find(
          (country) => country.country_id === participant.country_id
        );
        const award = data.awards.find((award) => award.award_id === participation.award_id);
